...
public class ModifyTeacherView implements View {
	...
	protected boolean submit(HttpServletRequest request,
		...
		if (submitValidator == null || submitValidator.validate(request)) {
			...
			String lock1 = ModelFactory.getUserSemaphore(orgModel.getEmail()), lock2;
			if (StringUtils.isNotEmpty(id)) {
				...
				lock2 = ModelFactory.getUserSemaphore(teacherModel.getEmail());
			} else {
				teacherModel = new TeacherModel();
				teacherModel.setOrganization(orgModel);
				lock2 = ModelFactory.getUserSemaphore(email);
			}

			boolean safe = true;
			if (objc.ext().setSemaphore(lock1, 0)
					&& objc.ext().setSemaphore(lock2, 0)) {
				try {
					safe = StringUtils.equals(email, teacherModel.getEmail())
							|| !objc.queryByExample(
					...
					objc.ext().releaseSemaphore(lock1);
					objc.ext().releaseSemaphore(lock2);
				}
			}

			Messages.addError(request, "email", safe ? userLockedMessage
